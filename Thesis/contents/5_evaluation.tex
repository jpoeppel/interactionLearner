...
The model are supposed to learn object manipulation through interaction.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{LearningCurveinteractionModel_C_0E20FoldsAllACSF.pdf}
...
The positional prediction error starts around \m{0.26} with a standard deviation of around \m{0.17} when the model is only trained with a single training example. The prediction performance improves to roughly \m{0.08} with a standard deviation of around \m{0.04}.
The predicted orientation error starts at an average of about \rad{0.81} with standard deviation of \rad{0.51} but improves to around \rad{0.25} with standard deviation of \rad{0.08} when increasing the number of random training examples.
The performances for position and orientation do not increase much more when training on 30 runs instead of 20. (\m{0.10} $+/-$ \m{0.07} for position after 20 runs and \rad{0.278} $+/-$ \rad{0.11} for orientation).
The actuator position predictions start with an error of \m{0.05} with a standard deviation of \m{0.03} and go down to \m{0.03} with a standard deviation of \m{0.02} already after 10 training runs.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{LearningCurvegateModel_C_0E20FoldsAllGateF.pdf}
...
The positional prediction error over all testing positions (blue error bars) starts at around \m{0.23} with a standard deviation of \m{0.15} for only one training run in the object state with gating function model. By increasing the number of random training runs to 30, the positional error reduces continuously to around \m{0.03} with standard deviation of only around \m{0.03}.
The predicted orientation error starts at around \rad{0.82} with standard deviation of \rad{0.63}.
...
The predicted errors are not as symmetric as in the other model. %TODO significance?
...
Due to the good performance both models exhibit in this scenario, a statistical evaluation is pointless. Instead figure \ref{fig:pushTaskSim2} shows images from two consecutive random test runs. %TODO which model/configuration
...
Afterwards, the interface queries the model for the next action primitive which is then send to the simulation. This is repeated until the target configuration is reached or a maximum number of steps of 3000 %TODO check
has been performed.
...
\end{table} %TODO redo and check!


\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{MoveToTargetInteractionT4F0Detail.pdf}
